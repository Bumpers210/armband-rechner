...
function carmaja_api_v2_publish_product(
    ...
    if (carmaja_api_publish_target() === 'production') {
        if (!carmaja_api_production_publish_enabled()) {
            carmaja_api_audit_best_effort('production_v2_publish_blocked', [
                'productId' => $productId,
                'deviceId' => $actor['tokenId'] ?? null,
                'result' => 'blocked',
            ]);
        ...
        if (!is_callable($GLOBALS['CARMAJA_API_PUBLISH_ADAPTER_V2'] ?? null)) {
            throw new CarmajaApiException(
                503,
                'V2-Produktionspublisher ist nicht freigegeben.',
                [],
