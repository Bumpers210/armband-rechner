...
const CARMAJA_PRODUCT_MODEL_VERSION = 2;
const CARMAJA_PRODUCT_MODEL_MIGRATION_CONFIRMATION = 'MIGRIEREN_V2';
...
function carmaja_api_publish_target(): string
{
    ...
    if (!is_string($target) || trim($target) !== 'production') {
...
function carmaja_api_private_dir(): string
{
    ...
    if (carmaja_api_normalize_path($configuredPath)
...
function carmaja_api_write_json_atomic(string $path, array $data): void
{
    ...
    try {
        ...
        if (!carmaja_api_json_values_equal($validated, $data)) {
...
/**
 * JSON macht keinen Unterschied zwischen den numerisch gleichen Werten 6 und 6.0.
 * Die Pruefung bleibt fuer alle nichtnumerischen Werte strikt, damit etwa "6" und
 * true niemals als gleichwertig behandelt werden.
 */
function carmaja_api_json_values_equal(mixed $left, mixed $right): bool
{
    $leftNumber = is_int($left) || is_float($left);
    $rightNumber = is_int($right) || is_float($right);
    ...
    if ($leftNumber || $rightNumber) {
        ...
        $decimal = is_float($left) ? $left : $right;

        if (!is_finite($decimal) || abs($decimal) > 9007199254740991) {
            return false;
        }

        return (int) $decimal === $integer && (float) $integer === $decimal;
    }
    ...
    if (is_array($left) || is_array($right)) {
        ...
        foreach ($left as $key => $value) {
            if (!carmaja_api_json_values_equal($value, $right[$key])) {
                return false;
            }
        }
...
function carmaja_api_with_product_write_barrier(callable $callback): mixed
{
    return carmaja_api_with_lock('product-model-write-barrier', $callback);
}
...
function carmaja_api_reject_legacy_product_fields(array $payload): void
{
    $legacyFields = array_values(array_intersect(
        array_keys($payload),
        ['braceletSize', 'stock', 'vintedUrl']
    ));
    ...
    throw new CarmajaApiException(
        409,
        'Die Anfrage verwendet ein nicht mehr unterstütztes Produktmodell.',
        array_fill_keys($legacyFields, 'Dieses V1-Feld wird nicht mehr akzeptiert.'),
        'legacy_product_model_unsupported'
    );
}

function carmaja_api_require_product_model_v2(array $payload): void
{
    $modelVersion = $payload['modelVersion'] ?? null;

    if (!is_int($modelVersion) || $modelVersion !== CARMAJA_PRODUCT_MODEL_VERSION) {
        throw new CarmajaApiException(
            409,
            'Die App verwendet keine unterstützte Produktmodellversion.',
            ['modelVersion' => 'Produktmodellversion 2 ist erforderlich.'],
            'product_model_version_unsupported'
        );
    }
}

function carmaja_api_validate_positive_measurement(mixed $value, string $field): int|float
{
    if ((!is_int($value) && !is_float($value))
        ...
        throw new CarmajaApiException(
            422,
            $field . ' ist ungültig.',
            [$field => 'Positive JSON-Zahl erwartet.'],
            'validation_failed'
        );
    }
...
function carmaja_api_validate_draft_payload(
    ...
    carmaja_api_reject_legacy_product_fields($payload);
    carmaja_api_require_product_model_v2($payload);
    ...
    $draft['modelVersion'] = CARMAJA_PRODUCT_MODEL_VERSION;
    ...
    $draft['braceletSizeCm'] = carmaja_api_validate_positive_measurement(
        $payload['braceletSizeCm'] ?? null,
        'braceletSizeCm'
    );
    $draft['pearlSizeMm'] = carmaja_api_validate_positive_measurement(
        $payload['pearlSizeMm'] ?? null,
        'pearlSizeMm'
    );
...
function carmaja_api_save_product(string $draftId, array $body, array $actor): array
{
    ...
    return carmaja_api_with_product_write_barrier(function () use (
        $draftId,
        $body,
        $expectedVersion,
        $actor
...
{
    if (($draft['modelVersion'] ?? null) !== CARMAJA_PRODUCT_MODEL_VERSION) {
        throw new CarmajaApiException(
            422,
    ...
    foreach (['braceletSizeCm', 'pearlSizeMm'] as $field) {
        try {
            carmaja_api_validate_positive_measurement($draft[$field] ?? null, $field);
        } catch (CarmajaApiException) {
            throw new CarmajaApiException(
                422,
                'Produkt ist noch nicht veröffentlichbar.',
                [$field => 'Positive Maßangabe ist erforderlich.'],
                'product_not_publishable'
            );
        }
    }
...
function carmaja_api_public_product_from_draft(array $draft): array
{
    ...
    $publicProduct = [
        ...
        'braceletSizeCm' => carmaja_api_validate_positive_measurement(
            $draft['braceletSizeCm'] ?? null,
            'braceletSizeCm'
        ),
        'pearlSizeMm' => carmaja_api_validate_positive_measurement(
            $draft['pearlSizeMm'] ?? null,
            'pearlSizeMm'
        ),
...
function carmaja_api_validate_status_transition(
    ...
    if ($newStatus === 'published') {
        $allowedCurrentStatuses = ['ready', 'published'];
...
function carmaja_api_publish(string $draftId, array $body, array $actor, string $newStatus): array
{
    ...
    return carmaja_api_with_lock('operation-' . hash('sha256', $operationId), function () use (
        ...
        try {
            ...
            $draft = carmaja_api_with_product_write_barrier(function () use (
                $draftId,
                $expectedVersion,
                $operationId,
                $newStatus,
...
function carmaja_api_operation_status(string $operationId): array
{
    ...
    if (($operation['status'] ?? null) === 'succeeded'
        && ($operation['publishTarget'] ?? null) === 'production'
        && $commitSha !== null
...
function carmaja_api_require_github_production_configuration(
    bool $requireEnabled = true
): void
{
    if (($requireEnabled && !carmaja_api_github_adapter_enabled())
        || ($requireEnabled && !carmaja_api_production_publish_enabled())
        || carmaja_api_publish_target() !== 'production') {
        throw new CarmajaApiException(
            503,
...
{
    carmaja_api_require_github_production_configuration(true);
}

function carmaja_api_github_token(bool $requireEnabled = true): string
{
    carmaja_api_require_github_production_configuration($requireEnabled);
...
function carmaja_api_github_repository(bool $requireEnabled = true): string
{
    ...
    if ($repository !== CARMAJA_PRODUCTION_REPOSITORY) {
...
function carmaja_api_github_branch(bool $requireEnabled = true): string
{
    ...
    if ($branch !== CARMAJA_PRODUCTION_BRANCH) {
        throw new CarmajaApiException(
            503,
            'GitHub-Zielbranch ist fuer den Produktionsadapter nicht erlaubt.',
            [],
...
): array
{
    ...
    if (is_callable($mock)) {
        ...
        if (!is_array($result)) {
            throw new CarmajaApiException(
                500,
                'GitHub-Adapter hat ein ungueltiges Mock-Ergebnis geliefert.',
                [],
                'github_mock_invalid'
            );
        }
...
function carmaja_api_github_deployment_status(string $commitSha): array
{
    ...
    $path = '/repos/' . $repository
        . '/actions/workflows/' . rawurlencode(CARMAJA_PRODUCTION_DEPLOY_WORKFLOW)
    ...
    $matchingRuns = array_values(array_filter(
        ...
        static fn (mixed $run): bool =>
            ...
            && ($run['head_branch'] ?? null) === CARMAJA_PRODUCTION_BRANCH
            && ($run['event'] ?? null) === 'push'
    ));
...
): string
{
    ...
    $decodedContent = base64_decode((string) $content['content'], true);

    try {
        $decoded = is_string($decodedContent)
            ? json_decode($decodedContent, true, 512, JSON_THROW_ON_ERROR)
            : null;
    } catch (JsonException) {
        $decoded = null;
    }
    ...
    if (!is_array($decoded) || array_is_list($decoded)) {
        throw new CarmajaApiException(
            409,
            'Öffentliche Produktprojektion ist nicht sicher lesbar.',
            [],
            'public_product_projection_invalid'
        );
    }

    $currentProducts = $decoded;

    $products = is_array($currentProducts['products'] ?? null)
        ? $currentProducts['products']
        : [];
    $rootKeys = array_keys($currentProducts);
    sort($rootKeys, SORT_STRING);

    if ($rootKeys !== ['products', 'version']
        || ($currentProducts['version'] ?? null) !== $targetDocumentVersion
    ...
    foreach ($products as $index => $product) {
        ...
        if ($isV2
            ...
            && !$sameProduct
            && (($product['sku'] ?? null) === ($publicProduct['sku'] ?? null)
                || ($product['slug'] ?? null) === ($publicProduct['slug'] ?? null))) {
            throw new CarmajaApiException(
                409,
                'SKU oder Produktadresse ist bereits öffentlich vergeben.',
                [],
                'public_product_identifier_conflict'
            );
        }
...
function carmaja_api_upload_images(string $draftId, array $body, array $actor): array
{
    ...
    try {
        $draft = carmaja_api_with_product_write_barrier(
            static fn (): array => carmaja_api_upload_image_transaction($draftId, $body, $actor)
        );
...
): array
{
    ...
    try {
        ...
        $imageInfo = carmaja_api_process_jpeg(
            ...
            $size,
            $temporaryFile,
            !($GLOBALS['CARMAJA_API_ALLOW_LOCAL_UPLOADS_FOR_VALIDATION'] ?? false)
        );
...
function carmaja_api_legacy_bracelet_size_to_cm(mixed $value): int|float|null
{
    if (is_int($value) || is_float($value)) {
    ...
    if (preg_match('/^([0-9]+(?:\.[0-9]+)?)\s*(?:cm)?$/iu', $normalized, $matches) !== 1) {
        return null;
    }
...
function carmaja_api_migrate_product_model_v2(bool $apply): array
{
    return carmaja_api_with_product_write_barrier(function () use ($apply): array {
        $directory = carmaja_api_path('drafts');
        $paths = glob($directory . DIRECTORY_SEPARATOR . '*.json') ?: [];
        sort($paths, SORT_STRING);
        $changes = [];
        $report = [
            'modelVersion' => CARMAJA_PRODUCT_MODEL_VERSION,
            'status' => $apply ? 'prepared' : 'dry_run',
            'writePerformed' => false,
            'draftsInspected' => 0,
            'draftsMigrated' => 0,
            'braceletSizeConverted' => 0,
            'braceletSizeNeedsManualReview' => 0,
            'pearlSizeMissing' => 0,
            'legacyFieldsRemoved' => [
                'braceletSize' => 0,
                'stock' => 0,
            ...
            'commerceInventoryOnHandUnchanged' => true,
            'backup' => null,
            'restoreDryRun' => null,
        ];
        ...
        foreach ($paths as $path) {
            ...
            $report['draftsInspected']++;
            $updated = $draft;
            // Commerce bleibt eine opake Struktur: Der Migrationsadapter kennt
            // weder onHand noch sonstige Commerce-Felder und darf sie nicht ändern.
            $commerceInventoryBefore = $draft['commerceInventory'] ?? null;
            $changed = false;
            $convertedBracelet = false;
            $manualBracelet = false;

            if (($updated['modelVersion'] ?? null) !== CARMAJA_PRODUCT_MODEL_VERSION) {
                $updated['modelVersion'] = CARMAJA_PRODUCT_MODEL_VERSION;
                $changed = true;
            }

            $braceletSize = $updated['braceletSizeCm'] ?? null;

            if ((!is_int($braceletSize) && !is_float($braceletSize))
                || !is_finite((float) $braceletSize)
                || (float) $braceletSize <= 0) {
                $converted = carmaja_api_legacy_bracelet_size_to_cm(
                    $updated['braceletSize'] ?? null
                );

                if ($converted === null) {
                    if (array_key_exists('braceletSizeCm', $updated)) {
                        unset($updated['braceletSizeCm']);
                        $changed = true;
                    }
                    $manualBracelet = true;
                } else {
                    $updated['braceletSizeCm'] = $converted;
                    $changed = true;
                    $convertedBracelet = true;
                }
            }

            $pearlSize = $updated['pearlSizeMm'] ?? null;

            if ((!is_int($pearlSize) && !is_float($pearlSize))
                || !is_finite((float) $pearlSize)
                || (float) $pearlSize <= 0) {
                if (array_key_exists('pearlSizeMm', $updated)) {
                    unset($updated['pearlSizeMm']);
                    $changed = true;
                }
                $report['pearlSizeMissing']++;
            }

            foreach (['braceletSize', 'stock', 'vintedUrl'] as $legacyField) {
                if (array_key_exists($legacyField, $updated)) {
                    unset($updated[$legacyField]);
                    $report['legacyFieldsRemoved'][$legacyField]++;
                    $changed = true;
                }
            }

            if (!carmaja_api_json_values_equal(
                $commerceInventoryBefore,
                $updated['commerceInventory'] ?? null
            )) {
                throw new CarmajaApiException(
                    500,
                    'Commerce-Bestand wurde durch die Produktmigration verändert.',
                    [],
                    'commerce_inventory_changed'
                );
            }

            if ($convertedBracelet) {
                $report['braceletSizeConverted']++;
            }
            if ($manualBracelet) {
                $report['braceletSizeNeedsManualReview']++;
            }

            if ($changed) {
                $updated['version'] = ((int) ($updated['version'] ?? 0)) + 1;
                $updated['updatedAt'] = carmaja_api_now();
                $changes[] = [
                    'draftId' => (string) ($updated['draftId'] ?? ''),
                    'draft' => $updated,
                ];
            }
        }

        $report['draftsMigrated'] = count($changes);
        ...
        $restoreDryRun = carmaja_api_restore_backup((string) $backup['backup'], true);

        foreach ($changes as $change) {
            carmaja_api_save_draft($change['draft']);
        }

        $report['status'] = 'migrated';
        $report['writePerformed'] = true;
        $report['backup'] = $backup['backup'];
        $report['restoreDryRun'] = $restoreDryRun['status'] ?? null;
        $report['completedAt'] = carmaja_api_now();
        $reportPath = carmaja_api_path(
            'audit/product-model-v2-' . gmdate('Ymd-His') . '-' . bin2hex(random_bytes(4)) . '.json'
        );
        carmaja_api_write_json_atomic($reportPath, carmaja_api_target_document($report));
        carmaja_api_audit('product_model_v2_migrated', [
            'draftsMigrated' => $report['draftsMigrated'],
...
function carmaja_api_diagnose_environment(): array
{
    ...
    if (PHP_MAJOR_VERSION !== 8 || PHP_MINOR_VERSION !== 4) {
        throw new CarmajaApiException(
            503,
            'Die Produktions-API erfordert PHP 8.4.',
            [],
            'php_version_invalid'
        );
    }
    ...
    $productionPrivateDir = carmaja_api_required_directory_setting(
    ...
    if (carmaja_api_normalize_path($publicWebroot)
            !== carmaja_api_normalize_path($productionApiWebroot)
    ...
    foreach ($configuredRoots as $leftIndex => $leftPath) {
        foreach ($configuredRoots as $rightIndex => $rightPath) {
            ...
            if (carmaja_api_path_is_inside($leftPath, $rightPath)
                ...
                throw new CarmajaApiException(
                    503,
                    'Produktionspfade sind nicht sicher getrennt.',
                    [],
...
function carmaja_api_create_backup(): array
{
    return carmaja_api_with_lock('backup', function (): array {
        ...
        $staging = $backupRoot . DIRECTORY_SEPARATOR . '.creating-'
            . $name . '-' . bin2hex(random_bytes(6));
        carmaja_api_ensure_directory($staging);

        try {
            $marker = $source . DIRECTORY_SEPARATOR . 'environment.json';

            if (!is_file($marker)
                || !copy($marker, $staging . DIRECTORY_SEPARATOR . 'environment.json')) {
            ...
            foreach (CARMAJA_BACKUP_DIRECTORIES as $directory) {
                ...
                if (!is_dir($directorySource)) {
                    throw new CarmajaApiException(
                        503,
                        'Privater Datenbereich ist fuer die Sicherung unvollstaendig.',
                        [],
                        'backup_source_incomplete'
                    );
                }

                carmaja_api_copy_tree(
                    $directorySource,
                    $staging . DIRECTORY_SEPARATOR . $directory
                );
            }
...
function carmaja_api_backup_directory(string $backup): string
{
    if (preg_match(CARMAJA_BACKUP_NAME_PATTERN, $backup) !== 1) {
        throw new CarmajaApiException(422, 'Backup-Bezeichner ist ungueltig.');
    }

    $directory = carmaja_api_path('backups/' . $backup);
    $realDirectory = realpath($directory);

    if (!is_string($realDirectory)
        || !is_dir($realDirectory)
        || !carmaja_api_path_is_inside($realDirectory, carmaja_api_path('backups'))) {
        throw new CarmajaApiException(404, 'Backup wurde nicht gefunden.', [], 'backup_not_found');
    }

    return $realDirectory;
}

function carmaja_api_restore_backup(string $backup, bool $dryRun): array
{
    return carmaja_api_with_lock('backup', function () use ($backup, $dryRun): array {
        $source = carmaja_api_backup_directory($backup);
        $marker = carmaja_api_read_environment_marker($source);
        $manifest = carmaja_api_read_target_json(
            $source . DIRECTORY_SEPARATOR . 'manifest.json',
            [],
            'Backup-Manifest'
        );
        ...
        if ($marker !== 'production'
            ...
            || ($manifest['backup'] ?? null) !== $backup
            || ($manifest['directories'] ?? null) !== CARMAJA_BACKUP_DIRECTORIES) {
            throw new CarmajaApiException(
                409,
                'Backup passt nicht zur Produktionsumgebung.',
                [],
        ...
        foreach (CARMAJA_BACKUP_DIRECTORIES as $directory) {
            $candidate = $source . DIRECTORY_SEPARATOR . $directory;

            if (!is_dir($candidate)) {
                throw new CarmajaApiException(
                    409,
                    'Backup ist unvollstaendig.',
                    [],
                    'backup_incomplete'
                );
            }
        ...
        $private = carmaja_api_private_dir();
        $stagingRoot = carmaja_api_path(
            'restore-staging/' . $backup . '-' . bin2hex(random_bytes(6))
        );
        $rollbackRoot = carmaja_api_path(
            'restore-rollback/' . $backup . '-' . bin2hex(random_bytes(6))
        );
        carmaja_api_ensure_directory($stagingRoot);
        carmaja_api_ensure_directory($rollbackRoot);
        $replaced = [];

        try {
            foreach ($available as $directory) {
                carmaja_api_copy_tree(
                    $source . DIRECTORY_SEPARATOR . $directory,
                    $stagingRoot . DIRECTORY_SEPARATOR . $directory
                );
            }
            ...
            foreach ($restoreDirectories as $directory) {
                ...
                if (is_dir($live)) {
                    if (!rename($live, $rollback)) {
                        throw new CarmajaApiException(
                            500,
                            'Bestehende private Daten konnten nicht gesichert werden.',
                            [],
                            'restore_rollback_prepare_failed'
                        );
                    }
                ...
                if (!rename($staged, $live)) {
                    ...
                    throw new CarmajaApiException(
                        500,
                        'Backup konnte nicht atomar uebernommen werden.',
                        [],
                        'restore_promote_failed'
                    );
                }
                ...
                $replaced[] = [
                    'live' => $live,
                    'rollback' => $rollback,
                    'hadPrevious' => $liveMoved,
                ];
            }
        } catch (Throwable $error) {
            foreach (array_reverse($replaced) as $entry) {
                if (is_dir($entry['live'])) {
                    carmaja_api_remove_tree($entry['live']);
                }

                if ($entry['hadPrevious'] && is_dir($entry['rollback'])) {
                    @rename($entry['rollback'], $entry['live']);
                }
            }
            ...
            if (is_dir($stagingRoot)) {
                carmaja_api_remove_tree($stagingRoot);
            }
        }

        foreach ($replaced as $entry) {
            if ($entry['hadPrevious'] && is_dir($entry['rollback'])) {
                carmaja_api_remove_tree($entry['rollback']);
            }
        }
        if (is_dir($rollbackRoot)) {
            carmaja_api_remove_tree($rollbackRoot);
        }

        carmaja_api_audit('backup_restored', ['backup' => $backup]);
...
function carmaja_api_copy_tree(string $source, string $target): void
{
    ...
    foreach (new RecursiveIteratorIterator(
        ...
        if ($entry->isLink()) {
            throw new CarmajaApiException(500, 'Symbolische Links sind nicht erlaubt.');
        }
