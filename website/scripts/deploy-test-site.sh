...
validate_relative_path()
{
    ...
    case "$relative_path" in
        products.json|*/products.json|public-products.json|*/public-products.json|\
        click.php|*/click.php|_internal|_internal/*|*/_internal/*|\
        statistik|statistik/*|*/statistik/*|runtime-config.php|*/runtime-config.php|\
        ...
        .htpasswd|*/.htpasswd|test-website.htpasswd|*/test-website.htpasswd)
...
assert_equal "$CARMAJA_SITE_DOMAIN" "$EXPECTED_DOMAIN" 'Falsche Testdomain.'
assert_equal "$CARMAJA_TEST_WEBROOT" "$WEBROOT" 'Falscher Webroot.'
assert_equal "$CARMAJA_TEST_DEPLOY_WORKSPACE" "$WORKSPACE" 'Falscher Deployworkspace.'
assert_equal "$CARMAJA_PRODUCTION_PUBLISH_ENABLED" 'false' 'Produktionspublish ist nicht gesperrt.'
assert_equal "$CARMAJA_PRODUCTION_DEPLOY_ENABLED" 'false' 'Produktionsdeploy ist nicht gesperrt.'
...
mkdir "$lock_directory" 2>/dev/null || fail 'Ein anderer Testdeploy haelt den Lock.'
...
if [ "$CARMAJA_DEPLOY_ACTION" = 'mark_verified' ]; then
    [ -f "$current_manifest" ] || fail 'Es gibt keinen aktiven Testexport.'
...
if [ "$CARMAJA_DEPLOY_ACTION" = 'rollback' ]; then
    [ -f "$current_manifest" ] || fail 'Es gibt keinen aktiven Testexport fuer Rollback.'
    validate_manifest "$current_manifest" "$CARMAJA_COMMIT_SHA" "$CARMAJA_RELEASE_ID" 'false'
