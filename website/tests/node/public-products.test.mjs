...
test("erlaubte Statuswerte werden ohne Marktplatzdaten akzeptiert", async () => {
...
test("ungültiger Status und alle Vinted-Daten werden abgelehnt", async () => {
  await rejectsProduct(product({ status: "archived" }), /Unbekannter Produktstatus/);
  await rejectsProduct(
    product({ vintedUrl: "https://www.vinted.de/items/123-test" }),
